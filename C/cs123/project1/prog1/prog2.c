...
int main(int argc, char** argv)
{
	...
	*/
	...
	pid_t pid[N];FILE* f[N];//using pipes

	//output file
	createOutFile(argv[argc-1]);
	//pipe
	int* pipeout = createOutPipe();
	//creates traffic.txt with position from 0 to max(int i)
	createTrafficFile();
	...
	for(int i=0; i< N ; i++)
	{
		...
		{//Child code
			close(pipeout[0]);
			close(1);dup(pipeout[1]); // make pipe(write)--->stdout
	...
	*///while(wait(NULL) > 0);// this > was a <
	close(pipeout[1]);
	close(0);dup(pipeout[0]);
	char buffer[200];//int n;
